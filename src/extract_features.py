...
if __name__ == "__main__":
    ...
    data = []
    data.append(load_data('../data/human_mouse_data .json', 0))  # Human data (label=0)
    data.append(load_data('../data/bot_mouse_data.json', 1))     # Bot data (label=1)

    # Create DataFrame and save features
    df = pd.DataFrame(data)
    df.to_csv('../data/mouse_features.csv', index=False)
    print("Features saved to ../data/mouse_features.csv")
    print("\nFeature comparison:")
    print(df)
